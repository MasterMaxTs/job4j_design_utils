...
public class SimpleMenuTest {
    ...
    @Test
    public void whenSelectThanReturnSame() {
        assertEquals(
                menu.select("Купить продукты").get().getChildren(),
                List.of("Купить хлеб", "Купить молоко")
                );

        assertEquals(
                menu.select("Сходить в магазин").get().getChildren(),
                List.of("Купить продукты")
        );

        assertEquals(
                menu.select("Покормить собаку").get().getChildren(),
                List.of()
        );
    }
